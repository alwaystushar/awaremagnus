...
function bindDynamicFormListeners() {
    ...
    logoInput.addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        document.getElementById(
          "logoPreview"
        ).innerHTML = `<img src="${reader.result}" class="max-w-full max-h-[140px] object-contain rounded-xl">`;
      };
  ...
  if (fileUrlInput) {
    fileUrlInput.addEventListener("input", (e) =>
      updatePreviewFromUrl(e.target.value.trim())
    );
  }
  ...
  if (radioUpload && radioUrl) {
    ...
    function switchToUpload() {
      ...
      spanUpload.classList.add(
        "bg-blue-50",
        "border-blue-100",
        "text-blue-600"
      );
      spanUrl.classList.remove(
        "bg-blue-50",
        "border-blue-100",
        "text-blue-600"
      );
    ...
    radioUrl.addEventListener("change", () => {
      if (radioUrl.checked) switchToUrl();
    });
  }
  ...
  if (submitBtn && form) {
    submitBtn.onclick = (ev) => {
      ...
      alert(
        "Form data logged to console. Replace submit handler to upload to your API."
      );
    };
  }
  ...
  if (cancelBtn) {
    cancelBtn.onclick = () => {
      document.getElementById(
        "leftForm"
      ).innerHTML = `<p class="text-gray-400">Select a card above to load the form...</p>`;
      setLogoPlaceholder();
      setFilePlaceholder();
...
function setLogoPlaceholder() {
  document.getElementById(
    "logoPreview"
  ).innerHTML = `<svg class="w-12 h-12 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="3" width="18" height="18" rx="4" /><circle cx="8.5" cy="8.5" r="1.5" /><path d="M21 15l-5-5-7 7"/></svg>`;
}
