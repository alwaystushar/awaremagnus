...
// Sucess chart 
  ...
  // (NFC Viewed + Target Compromised) - Links Clicked
  const total = NFCViewed + linksClicked + targetCompromised;
  const successFormula = (NFCViewed + targetCompromised) - linksClicked;
  const successRate = Math.max(0, Math.round((successFormula / total) * 100)); // Prevent negative %

  // Update left labels dynamically
  document.getElementById('NFCViewedText').innerText = `${NFCViewed} People`;
  ...
  var successOptions = {
    series: [NFCViewed, linksClicked, targetCompromised],
    chart: {
      type: 'donut',
      height: '100%'
    },
    labels: ['NFC Viewed', 'Links Clicked', 'Target Compromised'],
...
new ApexCharts(document.querySelector("#segmentsChart"), segmentsOptions).render();
    ...
    var reportOptions = {
      ...
      labels: ['NFC Opened Not Reported', 'NFC Opened & Reported', 'NFC Not Opened & Reported'],
